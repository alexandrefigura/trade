...
class AlertSystem:
    ...
    async def send_startup_alert(self, mode: str = "PAPER"):
        """Envia alerta de inicialização"""
        message = f"Sistema iniciado em modo {mode}"
        await self.send_alert("Sistema Iniciado", message, "info", force=True)
    
    async def send_shutdown_alert(self, reason: str = "Manual"):
        """Envia alerta de desligamento"""
        message = f"Sistema finalizado - Motivo: {reason}"
        await self.send_alert("Sistema Finalizado", message, "warning", force=True)